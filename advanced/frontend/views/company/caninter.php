...
<dl class="company_center_aside">
 ...
 <?php foreach($arr as $key => $val) {?>
		                        		<li data-id="1686182" name='li<?=$val['user_id']?>' class="onlineResume">
			                                <label class="checkbox">
			                                    <input type="checkbox">
			                                    <i></i>
			                                </label>
			                                ...
			                                <div class="resumeIntro">
			                                        <h3 class="unread"><a target="_blank" title="预览jason的简历" href="resumeView.html?deliverId=1686182"><?=$val['real_name']?>的简历
			                                        	</a><em></em>
			                                        </h3> 
			                                      
			                                        <div> 
			                                        	<?=$val['real_name']?>  			                                        	 / <?=$val['sex']?> 			                                        	/ <?=$val['topDegree']?> 			                                        	/ <?=$val['workyear']?>  			                                                                          			                                        		<br>
			                                            	高级产品经理 · 上海辉硕科技有限公司 | 本科 · 北京大学</div>
			                                        <div class="jdpublisher">
				                                        <span>
				                                        	应聘职位：<a title="随便写" target="_blank" href="http://www.lagou.com/jobs/149594.html"><?=$job_id[$key]['post']?></a></span>
			                                        </div>
			                                    </div>
			                                    <div class="links">
			                                        <a where="<?=$val['user_id']?>" where1="<?=$job_id[$key]['id']?>" name='job'  href="javascript:void(0)">通知面试</a>
			                                        <a where="<?=$val['user_id']?>" where1="<?=$job_id[$key]['id']?>" name='nojob' href="javascript:void(0)">不合适</a>
			                                   	
			                                    </div>
			                                </div>
			                                <div class="contactInfo">
			                                	<span class="c9">电话：</span><?=$val['tel']?>   &nbsp;&nbsp;&nbsp;   
			                                    <span class="c9">邮箱：</span><a href=""><?=$val['email']?></a>
			                                </div>
			                            </li>
			                            <?php }?>
		                            		                    	</ul><!-- end .resumeLists -->
            ...
            <script>
            	$("a[name='job']").click(function(){
            		var user_id=$(this).attr('where')
            		var job_id=$(this).attr('where1')
            		//$("li[name=li"+user_id+"]").remove();
            		$.ajax({
            			url:"?r=company/updajob",
            	...
            	$("a[name='nojob']").click(function(){
            		var user_id=$(this).attr('where')
            		var job_id=$(this).attr('where1')
            		//$("li[name=li"+user_id+"]").remove();
            		$.ajax({
            			url:"?r=company/updajobs",
            			data:{user_id:user_id,job_id:job_id},
            			type:'post',
            			success:function(e){
                           if(e==1){
