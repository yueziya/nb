...
<!--[if lte IE 8]>
      ...
      <div class="content_l">
       <dl class="job_detail">
        <dt>
          <h1 title="内容运营">
            <em></em>
            <div><?=$data['firm']?>招聘</div>
            <?=$data['post']?>
          </h1>                    
          <a class="inline jd_collection" id='love' where='<?=$_GET['id']?>' href="javascript:void(0)" title="收藏"></a>
        ...
        <dd class="job_request">
         <span class="red"><?=$data['money']?></span>
         <span><?=strip_tags($data['firm'])?></span> 
         <span><?=$data['year']?> </span>
 ...
 <dd>
 <!-- inline -->
   <a href="javascript:void(0)" class=" btn fr btn_apply" id="button" bty="<?=$_GET['id']?>">投个简历</a> 
 </dd>
 <script>
   $("#button").click(function(){
    ...
    $.ajax({
      ...
      success:function(e){
        ...
        if(e==3){
          alert('投递成功');
          location.href="?r=resume/delivery";
        }
      }
    })
   })
...
<div id="sendNoResume" class="popup">
 ...
 <a class="btn" href="?r=resume/index" target="_blank">完善在线简历</a>
</td>
</tr>
</table>
</div><!--/#sendNoResume-->
